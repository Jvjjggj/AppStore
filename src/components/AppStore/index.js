...
const initialList = appsList.filter(i => i.category === 'SOCIAL')

class AppStore extends Component {
  // eslint-disable-next-line react/no-unused-state
  state = {activeList: initialList, tabId: tabsList[0].tabId}

  onClickFilter = id => {
    const filterData = appsList.filter(i => i.category === id)
    this.setState({activeList: filterData, tabId: id})
  }

  searchResults = event => {
    const inputTarget = event.target.value
    const {activeList} = this.state
    const filterOnSearch = activeList.filter(i =>
      i.appName.includes(inputTarget),
    )
    this.setState({activeList: filterOnSearch})
  }

  render() {
    const {activeList, tabId} = this.state

    return (
      <div className="mainContainer">
        <h1 className="mainHead">App Store</h1>
        ...
        <ul className="tabsContainer">
          {tabsList.map(i => (
            <TabItem
              ...
              key={i.tabId}
              isActive={tabId === i.tabId}
            />
          ))}
        </ul>
        <ul className="gamesContainer">
          {activeList.map(i => (
            <AppItem details={i} key={i.appId} />
          ))}
        </ul>
      </div>
    )
